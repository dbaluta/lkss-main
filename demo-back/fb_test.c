 ...
 * at /dev/fb0 by the DRM panel-mipi-dbi driver (with fbdev emulation).
 *
...
static int fb_open(struct fb_ctx *ctx, const char *path)
{
	...
	printf("Framebuffer: %ux%u, %u bpp, line_length=%u, smem_len=%zu\n",
	       ctx->vinfo.xres, ctx->vinfo.yres,
	       ctx->vinfo.bits_per_pixel,
	       ctx->finfo.line_length, ctx->size);
 ...
 * after the last write to /dev/fb0.  Calling FBIOPAN_DISPLAY with the same
