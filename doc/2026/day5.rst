...
- the **BMP280 sensor driver** (Day 4) provides live temperature and pressure,
...
.. code-block:: c
   ...
   static void st7789fb_update_display(struct st7789_priv *priv)
   {
       ...
       for (i = 0; i < priv->width * priv->height; i++)
           tx[i] = swab16(fb[i]);                     /* endianness fix */
...
.. code-block:: bash
   ...
   rmmod st7789              # the Day 3 driver binds to the same device!
   modprobe st7789fb
   dmesg | tail -2
...
The 1978 memory toy, rebuilt with your LEDs: the board flashes a growing
color sequence on the LEDs and screen; repeat it with SW1 (red), SW2 (green),
SW3 (blue). One mistake and it's game over.

- **Base**: random growing sequence, LED + on-screen playback, input
  checking, score.
- **Stretch**: playback speeds up each round; "attract mode" LED animation
  while idle; two-player duel (alternate turns, separate scores); sound via
  a GPIO buzzer if you can find one.
- **Framework used**: ``hal_leds()`` synchronized with screen colors, state
  machine across two ``lv_timer`` callbacks.
- **Code**: ``simon/simon.c``
